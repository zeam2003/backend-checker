...
export class AuthController {
  constructor(private readonly authService: AuthService, private readonly config: AppConfigService) {}
...
@UseGuards(JwtAuthGuard)
async getNextToExpireTickets(@Req() req: RequestWithUser): Promise<MyTicketsResponseDto> {
  const sessionToken = req.user?.sessionToken;
  const userId = req.user?.userId;
...
async testGlpiSimple(@Query('sessionToken') sessionToken: string, @Query('userId') userIdQuery?: string, @Req() req?: any) {
